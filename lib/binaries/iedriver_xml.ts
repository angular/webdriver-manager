...
export class IEDriverXml extends XmlConfigSource {
  ...
  getVersionList(): Promise<string[]> {
    return this.getXml().then(() => {
      return this.getIEDriverList();
    });
  }


  private getIEDriverList(): string[] {
  ...
  private getLatestIEDriverVersion(): BinaryUrl {
    let list = this.getIEDriverList();
  ...
  private getSpecificIEDriverVersion(inputVersion: string): BinaryUrl {
    let list = this.getIEDriverList();
    let itemFound = '';
